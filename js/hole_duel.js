...
(function(){
  ...
  function resetGame(){
    ...
    state.step = parseInt(stepSel.value,10) || 8;
    state.diff = diffSel.value || "easy";
    state.mode = modeSel.value || "mix";
    state.timeOn = !!timeOn.checked;
    state.timeSec = parseInt(timeSel.value,10) || 8;
  ...
  function setMusic(on){
    ...
    if(!state.bg){
      ...
      a.loop = true;
      a.volume = (parseInt(bgVol.value,10) || 35) / 100;
      a.play().catch(()=>{});
      state.bg = a;
    }
  }
  ...
  function syncVol(){
    if(state.bg){
      state.bg.volume = (parseInt(bgVol.value,10) || 35) / 100;
    }
  }
...
if(btnStart) btnStart.addEventListener("click", resetGame);
  ...
  // bindings
  okL.addEventListener("click", ()=>onAnswer("L"));
  okR.addEventListener("click", ()=>onAnswer("R"));

  inL.addEventListener("keydown", (e)=>{ if(e.key==="Enter") onAnswer("L"); });
  inR.addEventListener("keydown", (e)=>{ if(e.key==="Enter") onAnswer("R"); });

  // quick focus
  pitch.addEventListener("click", ()=>{ inL.focus(); });

  diffSel.addEventListener("change", resetGame);
  modeSel.addEventListener("change", resetGame);
  stepSel.addEventListener("change", resetGame);
  timeOn.addEventListener("change", resetGame);
  timeSel.addEventListener("change", resetGame);

  bgOn.addEventListener("change", ()=>setMusic(bgOn.checked));
  bgVol.addEventListener("input", syncVol);
