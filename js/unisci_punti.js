...
function shapeSun(){
  // sole più carino: cerchio + piccoli raggi (non troppo a punta)
  const rays = 12, outer = 0.95, inner = 0.78, rot = -Math.PI/2;
  const pts=[];
...
function shapeMoon(){
  // mezzaluna pulita: due archi che si uniscono senza incroci
  const outer = arcPoints(0, 0, 0.95, 0.95, -Math.PI/2, Math.PI/2, 26);
  const inner = arcPoints(0.45, 0, 0.75, 0.75, Math.PI/2, -Math.PI/2, 26);
  return [...outer, ...inner, outer[0]];
}

function shapeHeart(){
  // cuore “classico” (parametrico) — niente incroci/sovrapposizioni
  const pts=[];
  const N=90;
  ...
  const out = pts.map(p=>({ x:(p.x-cx)/s, y:-(p.y-cy)/s })); // y invertita per schermo
...
function shapeHouse(){
  // casa più riconoscibile (tetto + pareti + base)
  return [
    {x:-0.85,y: 0.15}, // gronda sinistra
    {x: 0.00,y:-0.95}, // punta tetto
    {x: 0.85,y: 0.15}, // gronda destra
    {x: 0.85,y: 0.95}, // base destra
    {x:-0.85,y: 0.95}, // base sinistra
    {x:-0.85,y: 0.15}  // chiudi
  ];
}
...
function redrawLines(){
  ...
  const ptsAll = currentSet.points.slice().sort((a,b)=>a.n-b.n);

  // ✅ traccia guida (sotto): disegna tutta la sagoma in modo leggero e tratteggiato
  if(ptsAll.length >= 2){
    ctx.save();
    ctx.lineWidth = 3;
    ctx.setLineDash([6, 8]);
  ...
  if(currentSet._closeLoop){
    const maxN = Math.max(...ptsAll.map(p=>p.n));
    if(done.size >= maxN && maxN >= 2){
      const first = ptsAll[0];
